...
import { rasterStats, STUDY_AREA_HA, type RasterLayerStats } from "@/lib/stats-mock";
...
/** A raster layer switched on, paired with the year its stats should describe. */
...
function countByLayer(features: LayerFeature[]): VectorLayerCount[] {
  ...
  for (const feature of features) {
    ...
    else counts.set(id, { id, name, color, count: 1, geometryKind: geometryKindOf(feature) });
  }
...
export function StatsPanel({
  ...
  const rasterBlocks = rasterLayers
    .map(({ entry, year }) => rasterStats(entry, year))
    .filter((s): s is RasterLayerStats => s !== null);
  const vectorCounts = countByLayer(vectorFeatures);
  // Photographic rasters (Ortho, FCC) are on the map but have no classes to
  // count — say so rather than silently omitting them.
  const photographic = rasterLayers.filter(({ entry }) => entry.isPhotographic);
